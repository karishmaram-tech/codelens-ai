...
st.title("💻 CodeLens AI: Architecture & Logic Mapper")
    ...
    if not st.session_state.code_context:
        ...
        with st.spinner("Llama agent parsing AST and mapping architecture paths..."):
            try:
                ...
                mermaid_data = None
                if "graph " in response.content:
                    match = re.search(r"```mermaid\s*([\s\S]*?)\s*```", response.content)
                    if match:
                        mermaid_data = match.group(1).strip()
                    else:
                        fallback_match = re.search(r"(graph (?:TD|LR|BU|RL)[\s\S]*?)(?=\n\n|$)", response.content)
                        if fallback_match:
                            mermaid_data = fallback_match.group(1).strip()
                if mermaid_data:
                    mermaid_data = re.sub(r'\s+([A-Z]\[)', r'\n    \1', mermaid_data)
                    mermaid_data = re.sub(r'([\]\)])\s+(-->)', r'\1\n    \2', mermaid_data)
                    mermaid_data = re.sub(r'(-->\s*[A-Z])', r'\1', mermaid_data)
                    mermaid_data = re.sub(r'^(graph\s+[A-Z]{2})\s*', r'\1\n    ', mermaid_data)
                    st.subheader("📊 Interactive Logic Flowchart")
                    st_mermaid.st_mermaid(mermaid_data)
